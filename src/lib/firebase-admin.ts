...
function loadServiceAccount(): ServiceAccount {
  ...
  if (filePath) {
    ...
    if (existsSync(absolute)) {
      const json = JSON.parse(readFileSync(absolute, "utf8")) as {
        project_id: string;
        client_email: string;
        private_key: string;
      };
  ...
  throw new Error(
    "Missing Firebase Admin credentials. Set FIREBASE_PROJECT_ID, FIREBASE_CLIENT_EMAIL, FIREBASE_PRIVATE_KEY on Vercel.",
  );
}
